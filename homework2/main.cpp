...
bool check_intersection(point *p, border curr_border) {
...
int get_landig_interval(point p, vector<border> borders) {
    float t1 = (-p.vy - sqrt(p.vy*p.vy - 4 * p.y*g)) / (2 * g);
    float t2 = (-p.vy + sqrt(p.vy*p.vy - 4 * p.y*g)) / (2 * g);
    float t;
    if (t1 >= 0) {
        t = t1;
    }
